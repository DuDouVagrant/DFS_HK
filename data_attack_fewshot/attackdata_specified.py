...
class AttackDataset_specified(InMemoryDataset):
    def __init__(self, root, name, attackmethod, ptb_rate, transform=None, pre_transform=None):  
        ...
        self.name         = name.split('-',1)[1] # Cora,Citeseer,Pubmed,Polblogs 没有lower()
        self.attackmethod = attackmethod
        self.ptb_rate     = ptb_rate

        # 数据的下载和处理过程在父类中调用实现
        super(AttackDataset_specified, self).__init__(root, transform, pre_transform)

        # 加载数据
        self.data, self.slices = torch.load(self.processed_paths[0], map_location='cpu', weights_only=False) #注意这里需要map_location
    ...
    def process(self):
        ...
        adj = torch.load(osp.join(self.raw_dir, '{}.pt'.format(self.attack_file)), map_location='cpu', weights_only=False) #注意这里需要map_location
