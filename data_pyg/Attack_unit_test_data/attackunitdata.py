...
class AttackUnitDataset(InMemoryDataset):
    def __init__(self, root, name, scenario, split, adaptive_attack_model, ptb_rate, transform=None, pre_transform=None):  
        ...
        # 数据的下载和处理过程在父类中调用实现
        super(AttackUnitDataset, self).__init__(root, transform, pre_transform)

        # 加载数据
        # self.data, self.slices = torch.load(self.processed_paths[0],map_location= 'cuda') #注意这里需要map_location
        self.data, self.slices = torch.load(self.processed_paths[0], map_location='cpu', weights_only=False) # 根据模型需要看要加载到cpu还是gpu上


    @property
    def raw_dir(self) -> str:
        path = osp.join(self.root, self.scenario, self.name, str(self.split), self.adaptive_attack_model)
        return osp.join(path, 'raw')

    @property
    def processed_dir(self) -> str:
        path = osp.join(self.root, self.scenario, self.name, str(self.split), self.adaptive_attack_model)
        return osp.join(path, '{}_processed'.format(self.ptb_rate))


    # 将函数修饰为类属性
    @property
    def raw_file_names(self):
        return ['{}_features.npy'.format(self.name), '{}_lablels.npy'.format(self.name),
                'idx_train.npy',
                'idx_val.npy',
                'idx_test.npy',
                'A_perturbed_budget_ptb_{}.pt'.format(self.ptb_rate)
               ]

    @property
    def processed_file_names(self):
        return ['data_{}_{}_{}.pt'.format(self.name, self.adaptive_attack_model, str(self.ptb_rate))]
    ...
    def process(self):
        ...
        adj = torch.load(osp.join(self.raw_dir, 'A_perturbed_budget_ptb_{}.pt'.format(self.ptb_rate)), map_location='cpu', weights_only=False) #注意这里需要map_location
        ...
        self.idx_train = np.load(osp.join(self.raw_dir, 'idx_train.npy'))
        self.idx_val = np.load(osp.join(self.raw_dir, 'idx_val.npy'))
        self.idx_test = np.load(osp.join(self.raw_dir, 'idx_test.npy'))
