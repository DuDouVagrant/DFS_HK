...
class GPPTPrompt(torch.nn.Module):
    def __init__(self, n_hidden, center_num, n_classes, device=None, filter_module=None):
    ...
    def _maybe_filter_edge_index(self, h, edge_index):
        ...
        filter_output = self.filter_module(filter_graph)
        edge_mask = None
        if isinstance(filter_output, dict):
            edge_mask = filter_output.get('edge_mask', None)
        else:
            edge_mask = getattr(filter_output, 'edge_mask', None)
    ...
    def weigth_init(self, h, edge_index, label, index):
        ...
        edge_index = self._maybe_filter_edge_index(h, edge_index)
